...
function registerIPCHandlers() {
  ...
  // Save embedded file (base64 data) to storage directory
  ipcMain.handle('fs:saveEmbeddedFile', async (event, base64Data, fileName, itemId) => {
    try {
      ...
      // Create unique filename with itemId prefix to avoid conflicts
  ...
  // Ensure item directory exists (for reference mode, only create folder for metadata)
  ...
  ipcMain.handle('storage:readLibrary', async () => {
    ...
    try {
      ...
      try {
        ...
        // Return null to trigger safety check in renderer
        return null;
      }
    } catch (error) {
...
app.whenReady().then(async () => {
  ...
  // Initialize BM25 Full-Text Search (SQLite FTS5)
  try {
...
ipcMain.handle('search:hybrid', async (event, { query, limit = 10, vectorWeight: paramVectorWeight, bm25Weight: paramBM25Weight, groupByDoc = true }) => {
  ...
  try {
    ...
    const combinedResults = Array.from(scoreMap.entries())
      ...
      .slice(0, limit)
      .map(([id, { score, vectorRank, bm25Rank, data }]) => ({
        id,
        text: data.text || '',
        score,
        vectorRank,
