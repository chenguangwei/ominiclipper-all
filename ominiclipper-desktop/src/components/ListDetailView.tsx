...
const ListDetailView: React.FC<ListDetailViewProps> = ({
  ...
  const handleRevealInFinder = async () => {
    ...
    if (!filePath) {
      console.log('[ListDetailView] No valid path, attempting recovery...');
      filePath = await recoverItemPath(contextMenuTarget);
    }

    if (filePath) {
      console.log('[ListDetailView] Revealing in Finder:', filePath);
      (window as any).electronAPI.showItemInFolder(filePath);
    } else {
      console.warn('[ListDetailView] Cannot reveal in Finder: no valid path found');
    }
  };
