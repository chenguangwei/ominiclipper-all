...
const SettingsDialog: React.FC<SettingsDialogProps> = ({
  ...
  useEffect(() => {
    console.log('[SettingsDialog] storagePath changed:', storagePath);
    setLocalStoragePath(storagePath);
  }, [storagePath]);

  console.log('[SettingsDialog] Render isOpen:', isOpen);
      ...
      <div className="relative w-[500px] max-w-[90vw] bg-surface-secondary rounded-xl shadow-2xl border border-[rgb(var(--color-border)/var(--border-opacity))] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-3 pt-4 border-t border-[rgb(var(--color-border)/0.1)]">
            <label className="text-sm font-medium text-content-secondary">Advanced</label>
            <div className="p-4 rounded-lg bg-surface-tertiary/50 border border-[rgb(var(--color-border)/0.1)] space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-content">Search Index</div>
                  <div className="text-xs text-content-secondary">Rebuild vector index for AI search</div>
                </div>
                <button
                  onClick={async () => {
                    ...
                    try {
                      await storageService.reindexAllItems((current, total) => {
                        if (btn) btn.innerText = `Indexing ${current}/${total}...`;
                      });
