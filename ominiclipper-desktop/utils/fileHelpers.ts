...
/**
 * Try to recover file path via Electron API
 */
async function tryRecoverFilePath(item: ResourceItem): Promise<string | null> {
...
export const getFileData = async (item: ResourceItem): Promise<ArrayBuffer> => {
  ...
  if (isBlobUrl) {
    ...
    const recoveredPath = await tryRecoverFilePath(item);
    if (recoveredPath) {
  ...
  if (!filePath) {
    ...
    filePath = await tryRecoverFilePath(item);
    if (filePath) {
